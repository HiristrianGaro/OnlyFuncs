...
function stampaSquadre($squadre)
{
	...
	foreach ($squadre AS $sigla => $nome)
	{
		echo "<tr><td>$sigla</td>
		      <td>$nome</td> 
		      <td><a href=\"index.php?op=updateS&id=$sigla&nome=$nome\"><span class=\"glyphicon glyphicon-pencil\"></span></a></td>
...
function modificaSquadra($cid,$siglaOld,$sigla,$nome)
{	
	$risultato = array("status"=>"ok","msg"=>"", "contenuto"=>"");
	...
	$nome= trim($nome);
	if (empty($siglaOld) || empty($sigla) || empty($nome))
	{
		$errore = true;
	...
	{	
		...
		if (isset($squadre[$sigla]) && $sigla!=$siglaOld)
		{
			$errore = true;
	...
	if (!$errore)
	{
	   $sql= "UPDATE squadre SET sigla = '$sigla', nome = '$nome'
	           WHERE sigla = '$siglaOld';";
	   $res=$cid->query($sql);
	   if ($res==1)
	   ...
	   {
		   $risultato["status"]="ko";
		   $risultato["msg"]="L'operazione di modifica è fallita". $sql . $cid->error;
	   }
	}
	else
	{
...
{	
	...
	if (!$errore)
	{
	   ...
	   {
		   $risultato["status"]="ko";
		   $risultato["msg"]="L'operazione di inserimento è fallita". $sql . $cid->error;
	   }
	}
	else
	{
...
function cancellaSquadra($cid, $squadra){
	$risultato = array("status"=>"ok","msg"=>"", "contenuto"=>"");
	...
	$sql = "DELETE FROM squadre where sigla = '$squadra';";
	  // echo $sql;
	  ...
	  if ($res==null) 
		{
			$risultato["msg"]= "Problema nella cancellazione di una squadra nel database: $cid->errno: $cid->error()<br/>";
			$risultato["status"]="ko";
		}
	  else
	  {  
...
function selezionaSquadre($squadre, $nomeS, $selected)
{
	echo "<select type=\"text\" class='form-control' name='$nomeS'>";
	
	foreach ($squadre AS $sigla => $nome)
	{
		$sel = ($sigla==$selected)?"selected":"";
		echo "<option value='$sigla' $sel>$nome</option>";
	}
	echo "</select>";
}
...
function stampaPartite($partite,$squadre)
{
	...
	if (count($partite)>0)
	{
		foreach ($partite AS $partita)
		{
			...
			echo "<td><a href=\"index.php?op=updateP&casa=$partita[casa]&ospite=$partita[ospite]&goalcasa=$partita[goalcasa]&goalospite=$partita[goalospite]\"><span class=\"glyphicon glyphicon-pencil\"></span></a></td>";
			echo "<td><a href=\"backend/deleteP.php?casa=$partita[casa]&ospite=$partita[ospite]\"><span class=\"glyphicon glyphicon-remove\"></span></a></td></tr>";
...
function cancellaPartita($cid, $casa, $ospite){
	$risultato = array("status"=>"ok","msg"=>"", "contenuto"=>"");
	...
	$sql = "DELETE FROM partite where squadraCasa = '$casa' and squadraOspite = '$ospite';";
	  // echo $sql;
	  ...
	  if ($res==null) 
		{
			$risultato["msg"]= "Problema nella cancellazione di una partita nel database: $cid->errno: $cid->error()<br/>";
			$risultato["status"]="ko";
		}
	  else
	  {  
...
{	
	...
	if (!$errore)
	{
	  ...
	  if ($res==null) 
		{
			$risultato["msg"]= "Problema nella memorizzazione del risultato sul database: $cid->errno: $cid->error()<br/>";
			$risultato["status"]="ko";
		}
	  else
	  {  
...
function modificaPartita($cid,$oldsq1,$oldsq2,$sq1,$sq2,$ris1,$ris2)
{	
	$risultato = array("status"=>"ok","msg"=>"", "contenuto"=>"");
	$msg="";
	$errore=false;
	...
	if (!$errore)
	{
	  ...
	  $sql = "update partite
	          set squadraCasa='$sq1', squadraOspite='$sq2', 
			  golcasa=$ris1, golospite=$ris2
			  where squadraCasa='$oldsq1' and squadraOspite='$oldsq2';";
	  echo $sql;
